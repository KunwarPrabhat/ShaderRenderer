...
/* To avoid conflict with some C++ standards. */
#ifdef __cplusplus
#include <stdint.h>
#define TE_FUN(...) reinterpret_cast<double(*)(__VA_ARGS__)>(reinterpret_cast<uintptr_t>(const_cast<void*>(n->function)))
#else
#define TE_FUN(...) ((double(*)(__VA_ARGS__))n->function)
#endif


#define M(i) te_eval(n->parameters[i])


/* Determine the arity of a function/closure type. */
#define ARITY(t) ((t) & 7)
/* Check if a type is a closure. */
#define IS_CLOSURE(t) ((t) & 16)
/* Check if a type is pure (can be optimized). */
#define IS_PURE(t) ((t) & 32)
/* Mask the type to get the base type. */
...
#ifdef __cplusplus
static te_expr *new_expr_wrapper(int type, const te_expr *p1) {
    const te_expr *p[] = {p1};
    return new_expr(type, p);
}
static te_expr *new_expr_wrapper(int type, const te_expr *p1, const te_expr *p2) {
    const te_expr *p[] = {p1, p2};
    return new_expr(type, p);
}
#define NEW_EXPR(type, ...) new_expr_wrapper(type, __VA_ARGS__)
#else
...
static te_expr *base(state *s) {
    ...
    switch (TYPE_MASK(s->type)) {
        ...
        case TE_CLOSURE0:
            ...
            if (IS_CLOSURE(s->type)) ret->parameters[0] = (void*)s->context;
            next_token(s);
            if (s->type == TOK_OPEN) {
                next_token(s);
                if (s->type != TOK_CLOSE) {
        ...
        case TE_CLOSURE1:
            ...
            ret->function = s->function;
            if (IS_CLOSURE(s->type)) ret->parameters[1] = (void*)s->context;
            next_token(s);
        ...
        case TE_CLOSURE5: case TE_CLOSURE6: case TE_CLOSURE7:
            ...
            ret->function = s->function;
            if (IS_CLOSURE(s->type)) ret->parameters[arity] = (void*)s->context;
            next_token(s);
...
double te_eval(const te_expr *n) {
    ...
    switch(TYPE_MASK(n->type)) {
        ...
        case TE_CLOSURE4: case TE_CLOSURE5: case TE_CLOSURE6: case TE_CLOSURE7:
            switch(ARITY(n->type)) {
                case 0: return TE_FUN(const void*)(n->parameters[0]);
                case 1: return TE_FUN(const void*, double)(n->parameters[1], M(0));
                case 2: return TE_FUN(const void*, double, double)(n->parameters[2], M(0), M(1));
                case 3: return TE_FUN(const void*, double, double, double)(n->parameters[3], M(0), M(1), M(2));
                case 4: return TE_FUN(const void*, double, double, double, double)(n->parameters[4], M(0), M(1), M(2), M(3));
                case 5: return TE_FUN(const void*, double, double, double, double, double)(n->parameters[5], M(0), M(1), M(2), M(3), M(4));
                case 6: return TE_FUN(const void*, double, double, double, double, double, double)(n->parameters[6], M(0), M(1), M(2), M(3), M(4), M(5));
                case 7: return TE_FUN(const void*, double, double, double, double, double, double, double)(n->parameters[7], M(0), M(1), M(2), M(3), M(4), M(5), M(6));
                default: return NAN;
            }
